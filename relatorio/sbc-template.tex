...
\cite{wild} mostrar que o número destes pares é hiper geometricamente distribuído ${Hyp(n-1,n-x,x-1)}$: para os ${n - x}$ maiores elementos nós aleatoriamente traçamos suas posições no \textit{array} e temos ${x - 1}$ posições na partição esquerda . Por conseguinte, o número esperado de pares é ${(n-x)(x-1)/(n-1)}$ dado que o pivô é ${x}$.
...
Portanto, para utilização em softwares comerciais ou industriais, recomenda-se a utlização do método de particionamento de \textit{Hoare}, ao invés do método de particionamento de \textit{Lomuto}.
